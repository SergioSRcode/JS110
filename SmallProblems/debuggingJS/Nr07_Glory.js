...
// our two test rolls result in two random values that produce the sample output below 
// (because each dice roll produces a random value, your output may differ). 
// The outcome of rolling 16 looks correct, but the output when we 
// rolled values 1 and 20 doesn't make sense. 
...
function targetRoll(characterValue) {
  ...
  switch (result) {
    case 1:
      automaticFail();
      break;                // break statement necessary
    case 20:
      automaticSuccess();
      break;                // break statement necessary
    default:
