...
function leftoverCubes(num) {
  ...
  for (let i = 0; i < Math.sqrt(cubesPossibleOnLayer); i++) {
    
    if (num === 1) {
      layerStructure.push(num);
      break;
    }

    layerStructure.push(i * i);
  }

  layerSum = layerStructure.reduce((acc, square) => acc + square, 0);
